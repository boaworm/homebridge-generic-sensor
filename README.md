# homebridge-generic-sensor Plugin

This [Homebridge](https://github.com/nfarina/homebridge) plugin can be used integrate any "sensor" you want, as long as it is exposed as a numeric REST endpoint, api into HomeKit.
...
1) Generalizing it - making it a generic status sensor (rather than dealing specifically with temperatures)
...
instructions.  
Then run the following command to install `homebridge-generic-sensor`

```
sudo npm install -g homebridge-generic-sensor
```

## Updating the sensor reading in HomeKit

The _'CurrentStatus'_ characteristic has the permission to `notify` the HomeKit controller of state 
changes. `homebridge-generic-sensor` supports two concepts to send status changes to HomeKit.

### The 'pull' way:

The 'pull' way is probably the easiest to set up and supported in every scenario. `homebridge-generic-sensor` 
...
When using the 'push' concept, the http device itself sends the updated value to `homebridge-generic-sensor` 
whenever values change. This is more efficient as the new value is updated instantly and 
`homebridge-generic-sensor` does not need to make needless requests when the value didn't actually change.  
However because the http device needs to actively notify the `homebridge-generic-sensor` there is more 
...
* `unit` \<string\> **optional** \(Default: **"float**\): Defines unit expected from the http server. The following 

    are available:
    * **"float"**: Using a floating point value
    * **"int"**: Using a (signed) integer value -value to value
    * **"uint"**: Using an (unsigned) integer value (0 to value)
    ...
    "accessories": [
        {
          ...
          "getUrl": {
            "url": "http://localhost/api/getStatus",
            "method": "GET"
          }
        }   
    ]
...
`homebridge-generic-sensor` can be used together with 
...
Down here is an example on how to configure `homebridge-generic-sensor` to work with your implementation of the 
